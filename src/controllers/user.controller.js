    ...
    validate(data) {
        ...
        const validationSchema = Joi.object({
            ...
            pseudo: Joi.string().alphanum().min(2).max(150).required().messages({
                ...
                "string.alphanum": "Le pseudo doit contenir des caractères alphanumériques",
            ...
            password: Joi.string().pattern(new RegExp('^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@#$%^&*()!+=_-])[A-Za-z\\d@#$%^&*()!+=_-]{8,30}$')).required().messages({
                'string.min': 'Le mot de passe doit contenir au moins {#limit} caractères',
                'string.max': 'Le mot de passe doit contenir au maximum {#limit} caractères',
